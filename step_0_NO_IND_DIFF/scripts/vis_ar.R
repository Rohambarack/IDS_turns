...
save_it(pp_1,"../plots/ar/step_0_PP_ar_mu.pdf")

######## check prior posterior updates for group level ar
...
pp_2 <- grid.arrange(
  ...
  pp_bg_visit %>%
    ...
    geom_vline(xintercept = 0)+
    theme_classic(),
  top = textGrob("Prior-Posterior, Group-level Effects, Articulation Rate Mu",gp=gpar(fontsize=15,font=3))
)

save_it(pp_2,"../plots/ar/step_0_PP_ar_mu_sd.pdf")
...
pp_3 <- grid.arrange(
  ...
  top = textGrob("Prior-posterior, Population-level effects, Articulation Rate Sigma",gp=gpar(fontsize=15,font=3))
)

save_it(pp_3,"../plots/ar/step_0_PP_ar_sigma.pdf")
