...
log_prior_function <- function(){
  ...
  log_priors <- c(
    ...
    ###mean Individual skills
    prior(normal(0,1),class = sd, coef = "CHI_MLU", group = Participant),
    prior(normal(0,1),class = sd, coef = "MotorSkills", group = Participant),
    prior(normal(0,1),class = sd, coef = "Socialization", group = Participant),
    ##mean Visit
...
y_median_f0 <- bf(median_f0 ~ 0 + ASD + ASD:Visit +  ASD:Socialization + ASD:MotorSkills + ASD:CHI_MLU + (1 + Visit + Socialization + MotorSkills + CHI_MLU |gr(Participant, by=ASD)),
...
saveRDS(median_model,"../models/step_2_median_f0_3.rds")